...
class ResearchAreasESA:
    def __init__(self, esa_db_path, ra_db_host, ra_db_user, ra_db_password, ra_db_name, research_areas=None,
                 research_area_wikis=None, research_area_vectors=None, cutoff_in_relation_to_max=0.75, top=None,
                 sort=True, tfidf_proportion=0.2, esa=None):
        if esa is None:
    ...
    def get_research_area_sim_matrix(self):
        ...
        for ra_id, category, topic in research_areas:
    ...
    def get_research_areas_with_dbp(self, text, tfidf_extractor, db_results):
        db_research_areas = []
        ...
        research_areas_with_sim, res_areas, categories, top_category, tokens = \
            self.get_research_area_similarities_from_text(text, tfidf_extractor)
